...
namespace GitHubStatistics {
  ...
  export class RepoStats {
    url: string =
      "https://func-api-qqr6sxmfuyb34.azurewebsites.net/api/GetData";
    getLatestStat() {
      ...
      $.ajax({
        ...
        jsonpCallback:
          "GitHubStatistics.RepoStats.parseGitHubStatisticsResponse",
        url: this.url,
    ...
    static parseGitHubStatisticsResponse(data) {
      ...
      $("#TotalDownloadCount").text(
        result.TotalDownloadCount.toLocaleString(undefined),
      );
      $("#LatestReleaseDownloadCount").text(
        result.LatestReleaseDownloadCount.toLocaleString(undefined),
      );
